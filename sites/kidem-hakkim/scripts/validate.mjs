...
for (const [i, c] of cases.entries()) {
  ...
  if (!Array.isArray(c.rights) || c.rights.length < 1) errors.push(`${id}: en az bir hak kalemi olmalı`);
...
console.log(`  hakkın var:${v.var || 0}  yok:${v.yok || 0}  duruma göre:${v.kosullu || 0}  · şaşırtıcı:${cases.filter((c) => c.surprising).length}`);
